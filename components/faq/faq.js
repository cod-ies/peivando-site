import { faqCopy, getFaqItems } from "./copy.js";
import {
  EMAIL,
  htmlLang,
  isRtl,
...
function harvestDe() {
  document.querySelectorAll("[data-i]").forEach(function (el) {
    faqCopy.de[el.getAttribute("data-i")] = el.innerHTML;
  });
}
...
export function applyFaqLanguage(lang) {
  ...
  const wa = whatsappHref(dict["cta.waText"] || faqCopy.de["cta.waText"]);
...
export function hydrateFaq() {
  harvestDe();
  applyFaqLanguage(preferredLang());
  document.querySelectorAll(".langs button").forEach(function (button) {
    button.addEventListener("click", function () {
      applyFaqLanguage(button.dataset.l);
    });
  });
...
export { faqCopy, getFaqItems } from "./copy.js";
export { FAQ_IDS, PACKAGES } from "./constants.js";
